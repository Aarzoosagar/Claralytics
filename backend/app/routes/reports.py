...
def generate_report(
    ...
    try:
        ...
        file_size = pdf_result["file_size_bytes"]
        ...
        report.storage_backend = pdf_result["storage_backend"]
        report.file_path = pdf_result.get("file_path")
        report.s3_key = pdf_result.get("s3_key")
        report.s3_bucket = pdf_result.get("s3_bucket")
...
def download_report(
    ...
    storage_backend = report.storage_backend or "local"
    ...
    if storage_backend == "s3":
        ...
        try:
            presigned_url = S3Service.generate_presigned_url(
                report.s3_key,
                filename=report.filename or "claralytics_report.pdf",
            )
    ...
    # ── local fallback (dev / legacy reports generated before S3 integration) ──
