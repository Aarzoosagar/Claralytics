...
class ReportService:
    ...
    def generate_pdf(
        ...
        """Build a full PDF report.

        The document is always rendered into an in-memory buffer first (ReportLab
        happily writes to a file-like object), then persisted:
          - STORAGE_BACKEND=s3    -> uploaded straight to S3, nothing touches disk
          - STORAGE_BACKEND=local -> written under app/reports/ (original behaviour)

        Returns a dict: {storage_backend, filename, file_size_bytes, file_path?, s3_key?, s3_bucket?}
        """
    ...
    [
        ...
        if settings.use_s3:
            ...
            s3_key = S3Service.build_report_key(user_id, filename)
            S3Service.upload_bytes(pdf_bytes, s3_key, content_type="application/pdf")
            logger.info(f"Report generated and uploaded: s3://{settings.S3_BUCKET_NAME}/{s3_key}")
            return {
                "storage_backend": "s3",
                "filename": filename,
